        ...
        <div className="container max-w-4xl">
          ...
          <Card className="p-8 mb-8 bg-gradient-to-r from-blue-50 to-sky-50 border-2 border-blue-300">
            <div className="text-center mb-8">
              <h2 className="text-3xl font-bold text-brand-sky mb-4">答題結果</h2>
              <div className="text-5xl font-bold text-blue-600 mb-2">
                {correctCount}/{gradeData.questions.length}
              </div>
              <p className="text-lg text-gray-600">
                正確率：{Math.round((correctCount / gradeData.questions.length) * 100)}%
              </p>
            </div>
          </Card>
          ...
          <Card className="p-8 mb-8">
            <h3 className="text-2xl font-bold mb-6 text-gray-800">答題詳情</h3>
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1">
                        <p className="font-semibold text-gray-800">
                          第 {idx + 1} 題 ({getDisplayText(question.subject, question.subject)})
                        </p>
                        <p className="text-gray-600 mt-1">{getDisplayText(question.question, question.subject)}</p>
                        <p className="text-sm mt-2">
                          您的答案：
                          <span className={isCorrect ? "text-green-600 font-bold" : "text-red-600 font-bold"}>
                            {selectedAnswers[idx] !== null ? getDisplayText(question.options[selectedAnswers[idx]!], question.subject) : "未作答"}
                          </span>
                        </p>
                        {!isCorrect && (
                          <p className="text-sm text-green-600 font-bold">
                            正確答案：{getDisplayText(question.options[question.correctAnswer], question.subject)}
                          </p>
                        )}
                      </div>
                    </div>
          ...
          <Card className="p-8 mb-8 bg-blue-50 border-2 border-blue-200">
            <h3 className="text-2xl font-bold mb-6 text-gray-800">完整答案</h3>
        ...
        <div className="mb-8">
          <div className="flex justify-between items-center mb-2">
            <span className="text-sm font-semibold text-gray-600">
              第 {currentQuestionIndex + 1} / {gradeData.questions.length} 題
            </span>
            <span className="text-sm font-semibold text-gray-600">
